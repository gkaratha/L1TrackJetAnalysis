...
void SetStyle(TH1F * histo,TString xaxis,TString yaxis){
...
TCanvas *canvas_1graph(TGraph * gr, TString canvas, TString Labelx,TString Labely,bool Logx=false, bool Logy=false){

  TCanvas *data_mc_canvas = new TCanvas(canvas,canvas,700,700);
  if (Logx) data_mc_canvas->SetLogx();
  if (Logy) data_mc_canvas->SetLogy();
  gStyle->SetPadBorderMode(0);
  gStyle->SetOptStat(0);
  gr->SetLineColor(1);
...
TCanvas *canvas_5graph(TGraph * gr1, TGraph *gr2 , TGraph * gr3,TGraph * gr4,TGraph * gr5, TString canvas, TString Labelx,TString Labely,TString leg1,TString leg2,TString leg3,TString leg4,TString leg5){
  ...
  TCanvas *canvas_2graph(TGraph * gr1, TGraph *gr2 , TString canvas, TString Labelx,TString Labely,TString leg1,TString leg2,bool Logx=false, bool Logy=false){
...
TCanvas *canvas_4graph(TGraph * gr1, TGraph *gr2 , TGraph * gr3,TGraph * gr4, TString canvas, TString Labelx,TString Labely,TString leg1,TString leg2,TString leg3,TString leg4){
  ...
  TLegend * leg = new TLegend(0.70,0.70,1,1);
  leg->AddEntry(gr1,leg1); leg->AddEntry(gr2,leg2); leg->AddEntry(gr3,leg3);
  leg->AddEntry(gr4,leg4);
  auto mgr=new TMultiGraph();
  mgr->Add(gr1); mgr->Add(gr2); mgr->Add(gr3); mgr->Add(gr4);
  mgr->SetTitle(" ;"+ Labelx+" ;"+ Labely);
  mgr->Draw("AL*"); leg->SetTextFont(42); leg->SetFillColor(kWhite);
  leg->SetLineColor(kWhite); leg->SetBorderSize(0); leg->Draw();
  cms2().Draw("sames");
...
TCanvas *canvas_2plot(TH1F * h1, TH1F * h2, TString canvas, bool Logy, bool norm,double miny, double maxy, TString Labelx,TString Labely, TString leg1,TString leg2,bool dashed=false, bool Errors=false){

  TCanvas *data_mc_canvas = new TCanvas(canvas,canvas,700,700);
  gStyle->SetPadBorderMode(0);
  gStyle->SetOptStat(0);

  SetStyle(h1,Labelx,Labely);
  SetStyle(h2,Labelx,Labely);
  h1->SetLineColor(kRed);
